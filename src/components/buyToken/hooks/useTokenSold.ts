    ...
    try {
      if (poolAddress && tokenDetails && ableToFetchFromBlockchain
          && ethers.utils.isAddress(poolAddress)
           ...
           if (contract) {
             let tokenSold : any = await contract.methods.tokenSold().call();
             tokenSold = new BigNumber(tokenSold).div(new BigNumber(10).pow(tokenDetails?.decimals || 18)).toFixed(DECIMAL_PLACES);
      ...
      console.log('retrieveTokenSold', err.message);
    }
